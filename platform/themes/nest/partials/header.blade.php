...
<head>
    ...
    <link rel="stylesheet"
          href="https://fonts.googleapis.com/css2?family={{ urlencode(theme_option('font_text', 'Lato')) }}:ital,wght@0,400;0,700;1,400;1,700&family={{ urlencode(theme_option('font_heading', 'Quicksand')) }}:wght@400;500;600;700&display=swap"
          rel="stylesheet">
...
<header class="header-area header-style-1 header-height-2 {{ $headerStyle }}">
                    ...
                    <div class="header-info header-info-right">
                        <ul>
                            @if (theme_option('hotline'))
                                <li>{{ __('Need help? Call Us:') }} &nbsp;<strong
                                        class="text-brand"> {{ theme_option('hotline') }}</strong></li>
                            @endif
                            ...
                            @if (count($currencies) > 1)
                                <li>
                                    <a class="language-dropdown-active" href="#">{{ get_application_currency()->title }}
                            ...
                            <div class="header-action-2">
                                    ...
                                    <div class="header-action-icon-2">
                                        <a href="{{ route('public.wishlist') }}">
                                            <img class="svgInject" alt="{{ __('Wishlist') }}"
                                                 src="{{ asset('storage/general/heart.svg') }}"/>
                                ...
                                <div class="header-action-icon-2">
                                    <a class="mini-cart-icon" href="{{ route('public.cart') }}">
                                        <img alt="{{ __('Cart') }}"
                                             src="{{ asset('storage/general/bas.svg')}}"/>
                                ...
                                <div class="header-action-icon-2">
                                    <a href="{{ route('customer.overview') }}">
                                        <img class="svgInject rounded-circle" alt="{{ __('Account') }}"
                                             src="{{ asset('storage/general/30.svg') }}"/>
                                    </a>
                                    <a href="{{ route('customer.overview') }}"><span
                                            class="lable ml-0"></span></a>
                                        ...
                                        <ul>
                                            @if (auth('customer')->check())
                                                ...
                                                @if (EcommerceHelper::isOrderTrackingEnabled())
                                                    <li><a href="{{ route('public.orders.tracking') }}"><i
                                                                class="fi fi-rs-location-alt mr-10"></i>{{ __('Order Tracking') }}
                                                        </a></li>
                                                @endif
                                                @if (EcommerceHelper::isWishlistEnabled())
                                                    <li><a href="{{ route('public.wishlist') }}"><i
                                                                class="fi fi-rs-heart mr-10"></i>{{ __('My Wishlist') }}
                                                        </a></li>
                                                @endif
                                                <li><a href="{{ route('customer.edit-account') }}"><i
                                                            class="fi fi-rs-settings-sliders mr-10"></i>{{ __('Update profile') }}
                                                    </a></li>
                                            ...
                                            @else
                                                <li><a href="{{ route('customer.login') }}"><i
                                                            class="fi fi-rs-user mr-10"></i>{{ __('Login') }}</a></li>
    ...
    <div
        class="header-bottom header-bottom-bg-color  @if (theme_option('enabled_sticky_header', 'yes') == 'yes') sticky-bar @endif"
        style="height: 66px;">
        <div class="container">
            <div class="header-wrap header-space-between position-relative">
                    ...
                    @if (theme_option('enabled_browse_categories_on_header', 'yes') == 'yes')
                        <div class="main-categories-wrap d-none d-lg-block">
                            <a class="categories-button-active" href="#">
                                <span class="menu-custom-svg">
                                    <img
                                        src="{{ asset('storage/general/menu.svg') }}" width="30"
                                        alt="{{ theme_option('site_title') }}">

                                </span> {!! BaseHelper::clean(__('<span class="et">Browse</span> All Categories')) !!}
                            ...
                            <div class="categories-dropdown-wrap categories-dropdown-active-large font-heading">
                                ...
                                @if (count($categories) > 10)
                                    <div class="more_categories"><span class="icon"></span> <span
                                            class="heading-sm-1">{{ __('Show more...') }}</span></div>
                                @endif
                            </div>
                        </div>
                    @endif
                ...
                @if (theme_option('hotline'))
                    <div class="hotline d-none d-lg-flex mt-5">
                        <p class="mt-5">{{ theme_option('hotline') }}</p>
                        <img src="{{ asset('storage/general/70.svg') }}" width="20px" height="20px" alt="hotline"/>
                    </div>
                @endif
                            ...
                            @if (EcommerceHelper::isWishlistEnabled())
                                <div class="header-action-icon-2">
                                    <a href="{{ route('public.wishlist') }}">
                                        ...
                                        <span class="pro-count white wishlist-count">@if (auth('customer')->check())
                                                {{ auth('customer')->user()->wishlist()->count() }}
                                            @else
                                                {{ Cart::instance('wishlist')->count() }}
                                            @endif</span>
            ...
            <div class="mobile-header-info-wrap">
                ...
                @if (is_plugin_active('language'))
                    <div class="single-mobile-header-info">
                        <a class="mobile-language-active" href="#"><i class="fi-rs-globe"></i> {{ __('Language') }}
                            ...
                            <ul>
                                ...
                                @foreach (Language::getSupportedLocales() as $localeCode => $properties)
                                    <li><a rel="alternate" hreflang="{{ $localeCode }}"
                                           href="{{ $showRelated ? Language::getLocalizedURL($localeCode) : url($localeCode) }}">{!! language_flag($properties['lang_flag'], $properties['lang_name']) !!} {{ $properties['lang_name'] }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                ...
                @if (count($currencies) > 1)
                    <div class="single-mobile-header-info">
                        <a class="mobile-language-active" href="#"><i class="fi-rs-money"></i> {{ __('Currency') }}
                            <span><i class="fi-rs-angle-down"></i></span></a>
                        <div class="lang-curr-dropdown lang-dropdown-active">
                            <ul>
                                @foreach ($currencies as $currency)
                ...
                @if (is_plugin_active('ecommerce'))
                    <div class="single-mobile-header-info">
                        <a href="{{ route('customer.login') }}"><i class="fi-rs-user"></i> {{ __('Log In / Sign Up') }}
                        </a>
                    </div>
                @endif
                @if (theme_option('hotline'))
                    <div class="single-mobile-header-info">
                        <a href="tel:{{ theme_option('hotline') }}"><i
                                class="fi-rs-headphones"></i> {{ theme_option('hotline') }}</a>
                    </div>
                @endif
            </div>
