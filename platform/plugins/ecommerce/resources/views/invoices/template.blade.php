...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('themes/nest/fonts/fontIran/fontIran.css') }}?v=1.1.0">
    @if (get_ecommerce_setting('using_custom_font_for_invoice', 0) == 1 && get_ecommerce_setting('invoice_font_family'))
        <link
            href="https://fonts.googleapis.com/css?family={{ urlencode(get_ecommerce_setting('invoice_font_family')) }}:400,500,600,700,900&display=swap"
            rel="stylesheet">
    @endif
    <style>
        body {
    ...
    <script type="text/javascript" src="{{asset('themes/nest/js/vendor/jquery-3.6.0.min.js')}}"></script>
...
<body onload="myPrint()" id="print-page">
...
@if (get_ecommerce_setting('enable_invoice_stamp', 1) == 1)
    ...
    @elseif (trim($order->payment->status->label()))
        <span
            class="stamp @if ($order->payment->status == \Botble\Payment\Enums\PaymentStatusEnum::COMPLETED) is-completed @else is-failed @endif">
...
<table class="invoice-info-container">
    <tr>
        <td>
            ...
            <p>{{ get_ecommerce_setting('store_address') }}, {{ get_ecommerce_setting('store_city') }}
                , {{ get_ecommerce_setting('store_state') }}
                , {{ EcommerceHelper::getCountryNameById(get_ecommerce_setting('store_country')) }}</p>
            <p>{{ get_ecommerce_setting('store_phone') }}</p>
            @if (get_ecommerce_setting('store_vat_number'))
                <p>{{ trans('plugins/ecommerce::ecommerce.setting.vat_number') }}
                    : {{ get_ecommerce_setting('store_vat_number') }}</p>
            @endif
        </td>
        <td>
            @php
    ...
    <tr>
        <td class="payment-info">
            ...
            @if ($order->payment->payment_channel == \Botble\Payment\Enums\PaymentMethodEnum::BANK_TRANSFER && $order->payment->status == \Botble\Payment\Enums\PaymentStatusEnum::PENDING)
                <div>
                    {{ trans('plugins/ecommerce::order.payment_info') }}:
                    <strong>{!! BaseHelper::clean(get_payment_setting('description', $order->payment->payment_channel)) !!}</strong>
                </div>
            @endif
        </td>
