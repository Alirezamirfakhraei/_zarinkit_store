...
abstract class ZarinpalPaymentAbstract
{
    ...
    /**
     * ZarinpalPaymentAbstract constructor.
     */
    ...
    {
        try {
            $merchantId = setting('payment_zarinpal_terminalId', '111');
            $zarinpalMode = setting('payment_zarinpal_mode');

            if ($zarinpalMode) {
                return Payment::config(['merchantId' => $merchantId, 'mode' => 'normal'])
                    ->via('zarinpal')
                    ->callbackUrl($this->returnUrl)->purchase(
                        (new Invoice)->amount($this->getTotalAmount()),
                        function ($driver, $transactionId) {
                            session(['zarinpal_payment_id' => $transactionId]);
                        }
                    )->pay();
            }
            return Payment::via('zarinpal')
                ->config(['merchantId' => $merchantId, 'mode' => 'sandbox'])
                ->callbackUrl($this->returnUrl)->purchase(
                    (new Invoice)->amount($this->getTotalAmount()),
                    function ($driver, $transactionId) {
                        session(['zarinpal_payment_id' => $transactionId]);
     ...
     * @return mixed zarinpal checkout URL or false
     * @throws Exception
     */
    public function createPayment($transactionDescription)
    {
        ...
        try {
            ...
            $config = [
                ...
                'mode' => $zarinpalMode ? 'normal' : 'sandbox',
            ];
            $response = Payment::via('zarinpal')
                ->config($config)
            ...
            $checkoutUrl = $response ? $response->getAction() : false;
        } catch (Exception $exception) {
            $this->setErrorMessageAndLogging($exception, 1);
            return false;
        }
        if ($checkoutUrl && session('zarinpal_payment_id')) {
//            session(['zarinpal_payment_id' => $paymentId]);
            $paymentId = session('zarinpal_payment_id');
            return $checkoutUrl;
        }
    ...
    public function getPaymentStatus(Request $request)
    {
        $paymentId = session('zarinpal_payment_id');
        try {
            $receipt = Payment::amount($request->get('amount') / 10)->transactionId($paymentId)->verify();
