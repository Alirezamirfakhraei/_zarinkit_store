...
class BaseHelper
{
    ...
    public function formatTime(Carbon $timestamp, ?string $format = 'j M Y H:i'): string
    {
        ...
        $isJalali = config('core.base.general.date_format.type') === 'jalali';
        if ($timestamp->lte($first)) {
            return '';
        }

        return $isJalali ? \Morilog\Jalali\Jalalian::forge($timestamp)->format($format) : $timestamp->format($format);
    }
